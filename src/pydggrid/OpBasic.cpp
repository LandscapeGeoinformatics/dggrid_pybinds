...
// by default only main and DGG operations are active
OpBasic::OpBasic (const std::map<std::string, std::string>& parameters)
      : DgApOperationPList (parameters),
        mainOp (*this, true), dggOp (*this, true), inOp (*this, true),
        outOp (*this, true), primarySubOp (nullptr)
{
}
...
OpBasic::initialize (bool force)
{
   ...
   mainOp.initializeOp();
   if (this->parameterData.empty()) { pList.loadParams(inFileName, false); }
   else
   {
       for (auto const parameterNode : this->parameterData)
       {
           if (this->pList.getParam(parameterNode.first))
           {
               this->pList.setParam(parameterNode.first, parameterNode.second);
           }
       }
   }
