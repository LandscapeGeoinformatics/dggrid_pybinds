...
class Input(InputTemplate):
        ...
        if isinstance(data, dict):
            ...
            if columns is not None and isinstance(columns, dict):
                ...
                if isinstance(columns["lat"], int):
                    map_t: Dict[str, str] = {self.cols[n]: list(data.keys())[n] for n in index_array}
    ...
    def _export_frame(self, frame: pandas.DataFrame) -> bytes:
        """
