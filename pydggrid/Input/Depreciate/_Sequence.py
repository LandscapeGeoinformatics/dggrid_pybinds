...
class Input(InputTemplate):
    ...
    def save_range(self, start: int, end: int, step: int = 1) -> None:
        ...
        :return: None
        """
        self.data.append(list(range(start, end, step)))
        self.records.save(self.data[len(self.data) - 1], DataType.INT)
    ...
    def save_list(self, data: List[int]) -> None:
        ...
        :return: None
        """
        self.data.append([int(n) for n in data])
        self.records.save(self.data[len(self.data) - 1], DataType.INT)
    ...
    def save_numpy(self, data: numpy.ndarray, column: [int, None] = None) -> None:
        ...
        self.data.append(list(data[:, column_t].tolist()))
        self.records.save(self.data[len(self.data) - 1], DataType.INT)
    ...
    def save_frame(self, data: [pandas.DataFrame, geopandas.GeoDataFrame], column: [int, str] = 0) -> None:
        ...
        self.data.append(list(data.iloc[:, index_t].tolist()))
        self.records.save(self.data[len(self.data) - 1], DataType.INT)
