...
class Input(Dataset):
    ...
    def __init__(self):
        ...
        super().register_call(bytes, self._Bytes)
        ...
        self.field: str = ""
        self.type: PointDataType = PointDataType.GDAL
    ...
    def save(self, records: [Any], definition: Any) -> None:
        ...
        :return: None
        """
        self.field = definition if isinstance(definition, str) else self.field
    ...
    def _Bytes(self, records: bytes, definition: None) -> None:
        """
        Writes bytes directly to record buffer
        :param records: Record Bytes
        :param definition: Definition object, not required in this instance
        :return: None
        """
        return self.write(records, DataType.SHAPE_BINARY)
    ...
    def _ShapeFile(self, file_path: pathlib.Path, file_bytes: bytes, definition: Any) -> None:
        ...
        extensions: List[str] = list(["shp", "shx", "dbf", "prj", "sbn", "sbx"])
        for extension_string in extensions:
            absolute_path: str = str(file_path.absolute())
            absolute_path_n: List[str] = absolute_path.split(".")
            absolute_path_n[-1] = extension_string
            absolute_path = ".".join(absolute_path_n)
            if os.path.isfile(absolute_path):
                file_object = open(absolute_path, "rb")
                file_bytes: bytes = file_object.read()
                file_object.close()
                #
                byte_array.append(DataType.STRING.convert_bytes(extension_string))
                byte_array.append(DataType.BINARY.convert_bytes(file_bytes))
                extension_count = extension_count + 1
        byte_array.insert(0, DataType.INT.convert_bytes(extension_count))
        self.write(b''.join(byte_array), DataType.SHAPE_BINARY)
    ...
    def _String(self, records: str, definition: [str, None] = None) -> None:
        ...
        if Library.is_geojson(records):
            return self._GeoJSON(geojson.loads(records), definition)
    ...
    def _GeoFrame(self, records: geopandas.GeoDataFrame, definition: None = None) -> None:
        ...
        """
        elements: List[Dict[str, Any]] = list([])
        columns: Dict[str, str] = self._GetColumns(definition)
        coordinates: List[Tuple[float, float]] = list([geom.coords[0] for geom in records.geometry])
        for coordinate_index, coordinate_set in enumerate(coordinates):
            elements.append({
                "x": coordinate_set[0],
                "y": coordinate_set[1],
                "id": str(coordinate_index) if columns["id"] not in records.columns
                else str(records[columns["id"]][coordinate_index]),
                "label": f"P{coordinate_index}" if columns["label"] not in records.columns
                else str(records[columns["label"]][coordinate_index])
            })
        return self._DataFrame(pandas.DataFrame(elements), None)
    ...
    def _Dictionary(self,
        ...
        if isinstance(records, dict):
            if Library.is_geojson(records):
                return self._GeoJSON(records, definition)
